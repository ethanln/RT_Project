...
CameraView::CameraView(Camera* _camera, vec3 _look_from, vec3 _look_at, int _dim_x, int _dim_y, float _fov)
{
	this->camera = _camera;
...
Ray CameraView::get_initial_ray(int _x, int _y, bool is_normalized, bool is_jitter)
{
	try
	{
		vec3 p1 = this->look_from;
		vec3 p2 = this->view_plane.at(_y).at(_x);
		vec3 direction = is_jitter ? RayUtil::CAST_RAY_JITTER(p1, p2, this->pixel_radius_x, this->pixel_radius_y, is_normalized) : RayUtil::CAST_RAY(p1, p2, is_normalized);
...
void CameraView::set_cam_pos(vec3 _new_pos)
{
	this->camera->position = _new_pos;
}

void CameraView::set_cam_orientation(vec3 _new_orientation)
{
	this->camera->orientation = _new_orientation;
}

vec3 CameraView::get_cam_pos()
{
	return this->camera->position;
}

vec3 CameraView::get_cam_orientation()
{
	return this->camera->orientation;
}

vec3 CameraView::get_look_from()
{
	return this->look_from;
}

vec3 CameraView::get_look_at()
{
	return this->look_at;
}
...
vec3 CameraView::get_pixel_coor(int _x, int _y)
{
...
CameraView CameraView::clone()
{
	try
	{
		...
		vec3 clone_look_from = vec3(this->look_from.x, this->look_from.y, this->look_from.z);
		vec3 clone_look_at = vec3(this->look_at.x, this->look_at.y, this->look_at.z);
...
{
	float z = MAGNITUDE(this->look_from - this->look_at);

	// Get the maximum x and y value of the view plane.
	float max_x = z / cos(TO_RADIANS(this->fov / 2.0f));
	float max_y = z / cos(TO_RADIANS(this->fov / 2.0f));

	// Get the minimum x and y value of the view plane.
	float min_x = -(z / cos(TO_RADIANS(this->fov / 2.0f)));
	float min_y = -(z / cos(TO_RADIANS(this->fov / 2.0f)));
	...
	for (unsigned int i = 0; i < this->dim_y; i++) {
		this->view_plane.push_back(vector<vec3>());

		for (unsigned int j = 0; j < this->dim_x; j++)
		{
			// Instantiate x,y coordinate position of the pixel index.
			this->view_plane.at(i).push_back(vec3(current_x, current_y, 0.0f));
